...
image_folder = r"Z:/personal_data/Filip_Liebsch/SP8_Biocenter_small/2025-04-23_floxed/R670A_D456A_project/"

# global variables
Gphn = "Gphn_labels.tif"
vGAT = "vGAT_labels.tif"
gamma2 = "gamma2_labels.tif"
neuron = "Neuron_labels.tif"
soma = "Soma_labels.tif"

Gphn_int = "Gphn_intensities.tif"
gamma2_int = "gamma2_intensities.tif"
...
def intensity(root, img, intensity_img):
    ...
    statistics = {
    name + '_mean':       [p.mean_intensity     for p in properties]
    }
...
for root, dirs, files in os.walk(image_folder):
    if Gphn in files:
        execute(root, Gphn, Gphn_int)
        classify(root, Gphn, vGAT)
        classify(root, Gphn, gamma2)
        classify(root, Gphn, neuron)
        classify(root, Gphn, soma)
        intensity(root, neuron, Gphn_int)
        intensity(root, soma, Gphn_int)
        size(root, neuron)
        size(root, soma)
        parameters(root, Gphn)
